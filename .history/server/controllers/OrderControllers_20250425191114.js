import Order from '../model/orderModel.js'; // Adjust the path if needed
import Admin from '../model/AdminModel.js'; // Adjust the path if needed
...
export const addOrder = async (req, res) => {
    try {
        const { orderName, description, phoneNumber } = req.body;

        // Validate required fields
        if (!orderName || !description || !phoneNumber) {
...
export const toggleOrderStatus = async (req, res) => {
    ...
    try {
        ...
        if (!status || !['complete', 'pending'].includes(status)) {
            return res.status(400).json({ error: 'Invalid order status.  Must be "complete" or "pending".' });
        }
        ...
        order.status = status;  // update the order
