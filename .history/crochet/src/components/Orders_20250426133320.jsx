...
import AddOrder from './componets/AddOrder'
const Orders = () => {
  ...
  const filteredOrders = orders.filter(order => {
    // Check if order.id and order.customer are valid before calling toLowerCase
    const matchesSearch =
      (order.phoneNumber && order.phoneNumber.toLowerCase().includes(searchQuery.toLowerCase())) ||
      (order.orderName && order.orderName.toLowerCase().includes(searchQuery.toLowerCase()));
    const matchesStatus = statusFilter === 'all' || order.status === statusFilter;
    return matchesSearch && matchesStatus;
  });

  const handlePageChange = (direction) => {
    // Handle page change logic, this could be fetching from the store or updating pagination state
