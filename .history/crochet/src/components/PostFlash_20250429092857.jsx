...
const CreatePost = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/products/createFlash`,
        { ...formData, image },
        {
          headers: {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto mt-2 p-2 bg-white rounded-xl shadow-md">
      <h2 className="text-xl font-semibold text-center mb-2">Create New Post</h2>

      <form onSubmit={handleSubmit} className="space-y-4 flex flex-col lg:flex-row gap-4">
        {/* Image Upload */}
        <div
          className={`border-2 border-dashed rounded-lg p-4 text-center cursor-pointer transition 
            ${isDragging ? "border-blue-500 bg-blue-50" : "border-gray-300"} w-full lg:w-1/3`}
